...
describe Dragonfly::GoogleDataStore do

  # To run these tests, put a file ".google_spec.yml" in the dragonfly root dir, like this:
  # key: XXXXXXXXXX
  # secret: XXXXXXXXXX
  # enabled: true
  if File.exist?(file = File.expand_path('../../.google_spec.yml', __FILE__))
  ...
  if enabled
    ...
    before(:each) do
      @data_store = Dragonfly::GoogleDataStore.new(
  ...
  describe "registering with a symbol" do
    it "registers a symbol for configuring" do
      ...
      app.datastore.should be_a(Dragonfly::GoogleDataStore)
    end
  end
  ...
  describe "not configuring stuff properly" do
    it "should require a bucket name on write" do
      ...
      proc{ @data_store.write(content) }.should raise_error(Dragonfly::GoogleDataStore::NotConfigured)
    end
    ...
    it "should require a bucket name on read" do
      @data_store.bucket_name = nil
      proc{ @data_store.read('asdf') }.should raise_error(Dragonfly::GoogleDataStore::NotConfigured)
    end
